...
El likelihood es una función de cuan probable es un valor especifico de la variable $Y_i$ en donde se piensa que la proporción de estudiantes que apuestan
...
La distribución a posteriori representa una combinación convexa entre la verosimilitud de los datos ($Likelihood$) y la credibilidad a $priori$. 
...
```{r fig1, echo = FALSE, fig.width=10, fig.height=3, fig.cap="Distribucion del modelo bayesiano"}
...
Partiendo de una creencia inicial en la que todos los valores posibles de $\pi_a$ son igualmente probables, se observa en el gráfico @ref(fig:fig1) que la verosimilitud ($likelihood$) y la distribución $posterior$ son similares. Esto se debe a que la elección del $prior$ no afecta directamente al $posterior$, lo que genera que tenga una forma distribucional similar a la de la verosimilitud de los datos.
...
Del grafico @ref(fig:fig2) se puede ver como a medida que aumenta la probabilidad de que un estudiante mienta, se observó que la variabilidad del posterior es cada vez más chica, pero tiende a centrarse en un valor distinto al del parámetro de interés $\pi$.
...
A continuación se utilizó el método de Warner para calcular la
...
```{r fig3,echo=FALSE, fig.align = "center", fig.width = 5, fig.height = 3.5, warning=FALSE}
set.seed(1998)
