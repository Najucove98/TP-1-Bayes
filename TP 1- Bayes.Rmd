...
Las apuestas en línea han ganado una creciente popularidad entre los
adolescentes, impulsadas por la accesibilidad de plataformás digitales y
la constante exposición a la publicidad en redes sociales y eventos
deportivos. Donde en un principio se plantean que tienen el "control"
cuando la realidad termina siendo mucho más complejas, cuando se quedan
enredados en una nube de rachas ganadoras y cuando empiezan a perder
dinero siguen en el juego con la esperanza de recuperar.

Además de esto el poder realizar estudios sobre el tema es complejo,
porque a muchos adolescentes les causa verguenza el admitir que
practican dichos juegos; por lo tanto se crearon diversos métodos de
encuesta para reducir los sesgos por la mentira que pueden proporciónar
los usuarios o por el simple hecho de no querer responder, en este
trabajo se podran ver reflejados dos de estos métodos, como funciónan y
el análisis correspondiente con cada uno de los métodos.

\newpage

## Indice

1.[**Análisis de estudio considerando la mentira al responder las
preguntas**](#Objetivo-1)

2.[**Análisis de los apostadores utilizando el método de
Warner**](#Objetivo-2)

3.[**Análisis de los apostadores utilizando el método de
Greenberg**](#Objetivo-3)

4.[**Conclusiones finales sobre los métodos de respuesta
aleatorizada**](#Objetivo-4)

\newpage

### Análisis de estudio considerando la mentira al responder las preguntas {#Objetivo-1}

Se sabe que este es un tema que en muchos alumnos puede llegar a ser un
tema de verguenza porque algunos han llegado a perder mucho dinero, por
lo que muy posiblemente al hacerles la pregunta "¿Haz apostado en algun
momento?" posiblemente no sean honestos al responder.

Entonces entrando en materia del estudio se propone un modelo bayesiano,
ya que se busca saber si el alumno miente en la pregunta dado que ya se
sabe la respuesta y la probabilidad(p) con la que le toque la pregunta,
para esto se toma la información obtenida $(Prior)$ y la que se piensa
que puede ser para llegar a una conclusion $(Likelihood)$ que hace
combinar la información obtenida junto con la supuesta $(Posterior)$.
...
-   $n$: Cantidad de estudiantes en la muestra.
