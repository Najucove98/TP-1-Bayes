...
De la Figura @ref(fig:fig2) se puede ver como a medida que aumenta la probabilidad de que un estudiante mienta, se observó que la variabilidad del $posterior$ es cada vez más chica, pero tiende a centrarse en un valor sesgado distinto al del parámetro de interés $\pi$.
...
$$\text{Donde Z = } \int_{0}^{1} [\pi_a p + (1 - \pi_a)(1 - p)]^{y^*} [(1 - \pi_a)p + (1 - \pi_a)(1 - p)]^{N - y^*} d\pi_a$$
$$\text{Aplicando sustitución:} \\ \lambda = \pi_a p + (1 - \pi_a)(1 - p) \\  d\lambda = 2p \ - \ 1 \ d\pi_a \implies d\pi_a = \dfrac{d\lambda}{2p-1}$$
