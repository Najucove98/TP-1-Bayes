...
4.[**Comparación entre los métodos y los niveles de mentira**](#Objetivo-4)
...
```{r fig1, echo = FALSE, fig.width=10, fig.height=3, fig.cap="Distribución del modelo bayesiano"}
# Concatenar graficos
combinado <- (plt_prior | plt_likelihood | plt_posterior)
...
\FloatBarrier
```
```{r fig2,echo=FALSE, fig.align = "center", warning=FALSE, fig.width = 7, fig.height = 3.2, fig.cap= "Posterior para diferentes niveles de mentira"}
set.seed(1998)
...
Del grafico @ref(fig:fig2) se puede ver como a medida que aumenta la probabilidad de que un estudiante mienta, se observó que la variabilidad del $posterior$ es cada vez más chica, pero tiende a centrarse en un valor sesgado distinto al del parámetro de interés $\pi$.
...
```{r fig3,echo=FALSE, fig.align = "center", warning=FALSE, fig.width=4.5, fig.height=3.2, fig.cap="Comparación entre los niveles de mentira"}
set.seed(1998)
...
ggplot(Niv_mentira, aes(x = reorder(Método, Cobertura), y = Cobertura)) +
  ...
  labs(subtitle = "Cobertura de intervalos del 90%",
    x = "Nivel de mentira",
    y = "Cobertura"
  ) +
  ylim(0, 1) +
...
$$P(\pi_a \mid y^*) =  \dfrac{\binom{N}{y^*} \lambda^{y^*} (1 - \lambda)^{N - y^*}}{\int_{0}^{1} \binom{N}{y^*} \lambda^{y^*} (1 - \lambda)^{N - y^*} \, d\lambda}\\$$
$$P(\pi_a \mid y^*) = \dfrac{[\pi_a p + (1 - \pi_a)(1 - p)]^{y^*} [(1 - \pi_a)p + (1 - \pi_a)(1 - p)]^{N - y^*}}{\int_{0}^{1} [\pi_a p + (1 - \pi_a)(1 - p)]^{y^*} [(1 - \pi_a)p + (1 - \pi_a)(1 - p)]^{N - y^*}}\\$$
$$P(\pi_a \mid y^*) = \dfrac{[\pi_a p + (1 - \pi_a)(1 - p)]^{y^*} [(1 - \pi_a)p + (1 - \pi_a)(1 - p)]^{N - y^*} (1 - 2p)}{Z}\\$$
$$\text{Donde Z = } \int_{0}^{1} [\pi_a p + (1 - \pi_a)(1 - p)]^{y^*} [(1 - \pi_a)p + (1 - \pi_a)(1 - p)]^{N - y^*} d(\pi_a p + (1 - \pi_a)(1 - p))$$
$$\text{Aplicando sustitución se tiene Z = }\int_{1-p}^{p} \lambda_W^{y^*} (1 - \lambda_W)^{N-y^*} \dfrac{d\lambda_W}{2p-1} $$
$$Z = \dfrac{1}{2p-1}[\int_{0}^{p} \lambda_W^{(y^*+1)-1} (1 - \lambda_W)^{(N-y^*+1)-1} d\lambda_W - \int_{0}^{1-p} \lambda_W^{(y^*+1)-1} (1 - \lambda_W)^{(N-y^*+1)-1} d\lambda_W]$$
...
Para sumar al análisis se hacen graficos de los $posterior$ utilizando
distintos valores de $p$, por lo que se simulan distintas muestras para
poder ver el comportamiento en cada $p$ propuesto

```{r fig4,echo=FALSE, fig.align = "center", fig.width = 5, fig.height = 3.5, warning=FALSE, fig.cap="Posterior para distintos valores de p"}
set.seed(1998)
...
Por otro lado, para los valores de $p$ más centrales, como $p = 0.45$ o $p = 0.6$, las distribuciones resultan más dispersas, reflejando una mayor incertidumbre en la estimación de $\pi_a$. En resumen, el valor de $p$ influye directamente en la precisión de la distribución a posteriori, por lo que debía ser cuidadosamente considerado al momento de implementar este tipo de técnicas en estudios empíricos. Además de la influencia del valor de $p$, también se debe considerar el rol que juega la aleatoriedad de las muestras obtenidas.
...
```{r ,echo=FALSE, warning=FALSE, message=FALSE, fig.align = "center", fig.width = 6, fig.height = 3.7}
...
-   $Q_1 \text{ : } \text{Pregunta 1 (¿Participas en apuestas en linea?). } P(Q)=p \ \text{conocido}$
...
$$\text{Prior}$$ $$ \pi_a \sim Beta(a = 1, b = 1) \\$$
$$\text{Likelihood}$$
$$y^*\text{| }\pi_a \sim \text{Binomial}(n, \lambda_G), \text{ con } \lambda_G \text{ función de } \pi_a \\$$
...
\clearpage
```
## Comparación entre los métodos y los niveles de mentira {#Objetivo-4}
...
```{r fig6,echo=FALSE, fig.align = "center", warning=FALSE, fig.width = 7, fig.height = 3.2, fig.cap="Posterior para los niveles de μ"}
set.seed(1998)
...
```{r,echo=FALSE, fig.align = "center", fig.width = 6.5, fig.height = 4, warning=FALSE}
set.seed(2198)
#método Greenberg
#Definimos una función para evaluar distintos caso de p con un prior uniforme
...
#Guardo la lista que devuelve la función para poder graficar
datos_warner_green <- data.frame(
    p = rep(pi_grid, times = 2),
    posterior = c(posterior_w[[1]], posterior_g[[1]]),
    Metodo = rep(c("Warner", "Greenberg"), each = 200))

# Crear el grafico con ggplot2 con los siguientes mapeos
post_w <- ggplot(datos_warner_green, aes(x = p, y = posterior, color = Metodo)) +
    geom_line() +
    geom_area(aes(fill = Metodo), alpha = 0.5, position = "identity") +
    scale_color_manual(values = c("#287271","#E76F51")) + # c("#D95F02", "#1B9E77")
...
```
```{r fig7,echo=FALSE, fig.align = "center", fig.width = 5, fig.height = 3.5, warning=FALSE, fig.cap="Posterior Greenberg y Warner"}
# Concatenar graficos
post_w 
```
...
```{r fig8,echo=FALSE, fig.align = "center", fig.width = 5, fig.height = 4, warning=FALSE, fig.cap="Comparación entre los distintos métodos"}
set.seed(1998)
